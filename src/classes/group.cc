...
void Group::del_escort(Pilot *pilot){
    if(this->escorts.size()>0)
    {
        for(unsigned int i = 0;i<this->escorts.size();i++)//Find the escort with the given pilots id, and erase it.
        {
            if(pilot->get_id() == escorts.at(i)->get_id()){
		std::vector<Pilot *>::iterator it = this->escorts.begin() + i;
                this->escorts.erase(it);
            }
        }
    }
