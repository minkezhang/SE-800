...
void PacketUtils::make_packet(
	...
	int packet_size = sizeof(uint32_t) + sizeof(string);
...
void PacketUtils::fill_obj_packet(protos::RenderedObj *obj_packet, Projectile* obj) {
	...
	protos::vector *accel_vector = obj_packet->mutable_accel();
	accel_vector->set_x(obj->get_a().at(0));
	accel_vector->set_y(obj->get_a().at(1));
	accel_vector->set_z(obj->get_a().at(2));
